...
def timeout(packet, socket, address, is_done):
    t = threading.Timer(0.5, reset, args=(packet, socket, address))
    if (is_done):
        t.cancel()
        return
...
# udt send and receive is just sendto and recvfrom

# extract packet contents, returns data
def extract(packet):
...
# deliver data to application layer (might not be needed)
